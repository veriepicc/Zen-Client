...
class SubChunkBlockStoragePaletted<3,3> : public ISubChunkBlockStoragePaletted { /* Size=0x6e0 */
  /* 0x0000: fields for ISubChunkBlockStoragePaletted */
  /* 0x0008 */ uint32_t mBlocks[410];
  /* 0x0670 */ AppendOnlyAtomicLookupTable<Block const *,8> mPalette;
  /* 0x06d8 */ bool mModDataFlag;
  static const uint64_t CAPACITY;
  static const uint64_t BLOCKS_PER_WORD;
  static const uint64_t BLOCK_MASK;
  
  SubChunkBlockStoragePaletted<3,3>(const BlockVolume&, int16_t, const gsl::span<Block const *,-1>&);
  SubChunkBlockStoragePaletted<3,3>(const Block&);
  SubChunkBlockStoragePaletted<3,3>(const SubChunkBlockStorage&);
  SubChunkBlockStoragePaletted<3,3>(IDataInput&, const BlockPalette&, bool);
  ...
  std::bitset<8> findUsedIndices() const;
  ...
  virtual ~SubChunkBlockStoragePaletted<3,3>();
  virtual void* __vecDelDtor(uint32_t);
  
  static uint64_t packedWordsCount(uint64_t);
  static uint64_t getBlocksPerWord(uint64_t);
  static bool hasPadding();
  static std::bitset<8> _filterPalette(const std::function<bool __cdecl(Block const &)>&, gsl::span<Block const * const,-1>);
};
