...
class SubChunkBlockStoragePaletted<2,2> : public ISubChunkBlockStoragePaletted { /* Size=0x458 */
  /* 0x0000: fields for ISubChunkBlockStoragePaletted */
  /* 0x0008 */ uint32_t mBlocks[256];
  /* 0x0408 */ AppendOnlyAtomicLookupTable<Block const *,4> mPalette;
  /* 0x0450 */ bool mModDataFlag;
  static const uint64_t CAPACITY;
  static const uint64_t BLOCKS_PER_WORD;
  static const uint64_t BLOCK_MASK;
  
  SubChunkBlockStoragePaletted<2,2>(const BlockVolume&, int16_t, const gsl::span<Block const *,-1>&);
  SubChunkBlockStoragePaletted<2,2>(const Block&);
  SubChunkBlockStoragePaletted<2,2>(const SubChunkBlockStorage&);
  SubChunkBlockStoragePaletted<2,2>(IDataInput&, const BlockPalette&, bool);
  ...
  std::bitset<4> findUsedIndices() const;
  ...
  virtual ~SubChunkBlockStoragePaletted<2,2>();
  virtual void* __vecDelDtor(uint32_t);
  
  static uint64_t packedWordsCount(uint64_t);
  static uint64_t getBlocksPerWord(uint64_t);
  static bool hasPadding();
  static std::bitset<4> _filterPalette(const std::function<bool __cdecl(Block const &)>&, gsl::span<Block const * const,-1>);
};
